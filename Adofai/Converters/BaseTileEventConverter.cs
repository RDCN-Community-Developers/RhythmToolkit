...
{
	internal class BaseTileEventConverter<TEvent>(ADLevel level, LevelReadOrWriteSettings inputSettings) : BaseEventConverter<TEvent>(level, inputSettings) where TEvent : BaseTileEvent
	{
		public override TEvent GetDeserializedObject(JObject jobj, Type objectType, TEvent? existingValue, bool hasExistingValue, JsonSerializer serializer)
		{
			...
			_canread = false;
			if (Utils.Utils.ADConvertToType(jobj["eventType"]?.ToObject<string>() ?? throw new IllegalEventTypeException(jobj["eventType"]?.ToObject<string>() ?? "UnknownType")) == typeof(CustomEvent))
			{
				existingValue = (TEvent)(object)new CustomTileEventConverter(level, settings).GetDeserializedObject(jobj, objectType, null, hasExistingValue, serializer);
			}
			else
			{
				jobj.Remove("floor");
