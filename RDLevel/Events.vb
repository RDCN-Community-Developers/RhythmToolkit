...
#End If
		...
		Public MustInherit Class BaseRowAnimations
			Inherits BaseRows
		End Class
		Public NotInheritable Class PlaySong
		...
		Public Class SetTheme
			...
			<JsonProperty>
			Public Overrides ReadOnly Property Rooms As New Rooms With {.EnableTop = False, .Multipy = True}
			Public Overrides Function ToString() As String
		...
		Public Class SetVFXPreset
			...
			End Enum
			<JsonProperty>
			Public Overrides ReadOnly Property Rooms As New Rooms With {.EnableTop = True, .Multipy = True}
			''' <summary>
			''' 预设
			''' </summary>
			''' <returns></returns>
		...
		End Class
		Public NotInheritable Class FloatingText
			Inherits BaseEvent
			<Flags>
			Public Enum OutMode
				FadeOut
			...
			Public Property Color As PanelColor = Drawing.Color.White
			Public Property Angle As INumberOrExpression
			Public Property Size As UInteger
			Public Property OutlineColor As PanelColor = Drawing.Color.Black
			Public Property Id As Integer
				...
				End Set
			End Property
			Public Property TextPosition As NumberOrExpressionPair = New NumberOrExpressionPair(New Number(50), New Number(50))
		...
		End Class
		Public NotInheritable Class AdvanceText
			Inherits BaseEvent
		...
		Public Class TagAction
			...
			Public Overrides ReadOnly Property Rooms As Rooms = Rooms.Default
			Public Overrides ReadOnly Property Tab As Tabs = Tabs.Actions
			Public Function ControllingEvents(level As RDLevel) As IEnumerable(Of BaseEvent)
				Dim L As New List(Of BaseEvent)
				If Action.HasFlag(Actions.All) Then
					For Each group In level.Tags.Where(Function(i) i.Key.Contains(ActionTag))
						L.AddRange(group.Value)
					Next
				Else
					For Each group In level.Tags.Where(Function(i) i.Key = ActionTag)
						L.AddRange(group.Value)
					Next
				End If
				Return L.AsEnumerable
			End Function
			Public Function ControllingEventsPadLeft(level As RDLevel) As IEnumerable(Of BaseEvent)
				Dim L As New List(Of BaseEvent)
				If Action.HasFlag(Actions.All) Then
					For Each group In level.Tags.Where(Function(i) i.Key.Contains(ActionTag))
						L.AddRange(group.Value)
				...
				Return L.AsEnumerable
			End Function
			Public Shared Function ControllingEvents(level As RDLevel, ParamArray tag As SpecialTag()) As IEnumerable(Of BaseEvent)
				Dim L As New List(Of BaseEvent)
				For Each group In level.Tags.Where(Function(i)
													   Dim Avaliable As Boolean = True
													   For Each item In tag
														   If Not i.Key.Contains("[" + tag.ToString + "]") Then
															   Return False
														   End If
													   Next
													   Return True
		...
		Public Class WindowResize
			...
			Public Property Ease As EaseType
		End Class
		Public NotInheritable Class PlayAnimation
			Inherits BaseDecorationActions
		...
		Public Class Tint
			...
			Public Property Opacity As Integer
			Public Property Tint As Boolean
			Public Property TintColor As PanelColor = Drawing.Color.White
			''' <summary>
			''' 持续时间
			''' </summary>
			Public Property Duration As Single
		...
		Public NotInheritable Class Move
			...
			End Function
		End Class
		Public NotInheritable Class SetVisible
			Inherits BaseDecorationActions
			...
			End Function
		End Class
		Public NotInheritable Class AddClassicBeat
			Inherits BaseBeats
			Enum Patterns
				ThreeBeat
				FourBeat
		...
		Public Class SetRowXs
			...
			Enum Patterns
				...
				None
			End Enum
			Private _pattern As New List(Of Patterns)(6)
			...
			End Property
			<JsonIgnore>
			Public Property PatternEnum As List(Of Patterns)
				Get
					Return _pattern
				End Get
				Set(value As List(Of Patterns))
					_pattern = value
				End Set
			End Property
		...
		Public Class MoveRoom
			...
			'	Get
			'		Return New Rooms(Y) With {.EnableTop = False, .Multipy = False}
			'	End Get
			'	Set(value As Rooms)
