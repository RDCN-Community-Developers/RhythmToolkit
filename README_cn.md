...
## 关于这个项目的方向与定位
这个项目最初命名为 `RhythmToolkit`，目标是为《节奏医生》开发一些小工具，用来简化关卡的处理什么的。  
随着项目逐渐完善，我发现大部分工作都集中在对《节奏医生》关卡模型的重新实现与优化上，发展方向逐步偏向成为其他工具的基础框架，相比之下原先那几个小工具反而显得微不足道了，以及后来我也尝试支持了《冰与火之舞》（Adofai）的关卡模型，让它不单单是为节奏医生开发的了（虽然主要还是节奏医生）。  
此外，由于 SkiaSharp 存在一定的兼容性问题，我计划将图像处理部分独立出去。基于以上原因，项目更名为 `RhythmBase`，并将工具性质的内容迁移至其他仓库。  
当然，你也仍然可以简称它为 `RDTK`！  
为了减少项目依赖并解决兼容性问题，我将一些我觉得实用性较强的功能封装迁移到了尚未发布的项目 `RhythmBase.Addition` 中，这部分也可以作为 `RhythmBase` 的使用参考示例。  