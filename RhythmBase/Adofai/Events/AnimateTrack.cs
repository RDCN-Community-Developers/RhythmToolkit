...
public class AnimateTrack : BaseTileEvent, ISingleEvent
{
	...
	/// </summary>  
	[RDJsonCondition($"$&.{nameof(TrackAnimation)} is not RhythmBase.Adofai.{nameof(TrackAnimationType)}.{nameof(TrackAnimationType.None)}")]
	public float BeatsAhead { get; set; } = 3;
	/// <summary>  
	/// Gets or sets the number of beats behind for the animation.  
	/// </summary>  
	[RDJsonCondition($"$&.{nameof(TrackDisappearAnimation)} is not RhythmBase.Adofai.{nameof(TrackDisappearAnimationType)}.{nameof(TrackDisappearAnimationType.None)}")]
