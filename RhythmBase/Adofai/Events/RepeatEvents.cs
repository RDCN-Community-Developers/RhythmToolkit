...
public class RepeatEvents : BaseTileEvent
{
	...
	/// </summary>  
	[RDJsonCondition($"$&.{nameof(RepeatType)} == RhythmBase.Adofai.{nameof(Adofai.RepeatType)}.{nameof(RepeatType.Beat)}")]
	public float Interval { get; set; } = 1f;
	/// <summary>  
	/// Gets or sets a value indicating whether the event should execute on the current floor.  
	/// </summary>  
	[RDJsonCondition($"$&.{nameof(RepeatType)} == RhythmBase.Adofai.{nameof(Adofai.RepeatType)}.{nameof(RepeatType.Floor)}")]
