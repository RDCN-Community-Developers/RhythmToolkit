...
#End If
	...
	'            Return value
	'        End Function
	'        Friend Function Serialize() As String Implements RDExpression.Serialize
	'            Return value
	'        End Function
	'        Public Function TryGetValue() As Single? Implements RDExpression.TryGetValue
	'            Return value
	'        End Function
	...
	'        Public X As RDExpression
	'        Public Y As RDExpression
	...
	'            End If
	'        End Sub
	'        Public Shared Widening Operator CType(value As (x As RDExpression, y As RDExpression)) As RDPointE
	'            Return New RDPointE(value.x, value.y)
	'        End Operator
	'        Public Shared Widening Operator CType(value As (x As String, y As String)) As RDPointE
	'            Return New RDPointE(value.x, value.y)
	'        End Operator
	'        Public Overrides Function ToString() As String
	'            Return $"{{{X},{Y}}}"
	...
	'        End Function
	'        Public Shared Widening Operator CType(v As Linq.JArray) As RDPointE
	'            Return New RDPointE(v(0).ToString, v(1).ToString)
	'        End Operator
	'    End Structure
	...
	'            Return (e.X, e.Y)
	'        End Operator
	'        Public Shared Widening Operator CType(e As RDPoint) As RDPointE
	'            Return New RDPointE(e.X, e.Y)
	'        End Operator
	...
	'            Return New RDPoint(e.X, e.Y)
	'        End Operator
	'        Public Shared Narrowing Operator CType(e As RDPointE) As RDPoint
	...
	Public Structure RDBeat
		...
		Friend ReadOnly Property baseLevel As LevelElements.RDLevel
			Get
		...
		Public Shared Operator >(a As RDBeat, b As RDBeat) As Boolean
			Return FromSameLevel(a, b, True) AndAlso a._beat > b._beat
		End Operator
		...
		Public Overrides Function GetHashCode() As Integer
			Return HashCode.Combine(Me._beat, Me.baseLevel)
		End Function
	End Structure
	...
	Public Class PanelColor
		...
		End Function
	End Class
	<JsonConverter(GetType(Converters.RoomConverter))>
	Public Class Rooms
		<Flags>
		Enum RoomIndex
			None = &B0
...
._data = RoomIndex.RoomNotAvaliable
	...
	Public Class LimitedList(Of T)
		...
		End Function
	End Class
	'<JsonConverter(GetType(Converters.LimitedListConverter(Of Object)))>
	'Public Class LimitedList(Of T)
	'    Implements ICollection(Of T)
	'    Private ReadOnly list As List(Of (value As T, isDefault As Boolean))
	'    <JsonIgnore>
	...
	Public MustInherit Class OrderedEventCollection
		...
		Public Overridable Function Contains(item As BaseEvent) As Boolean Implements ICollection(Of BaseEvent).Contains
			...
			Return EventsBeatOrder.ContainsKey(item.Beat.BeatOnly) AndAlso EventsBeatOrder(item.Beat.BeatOnly).Contains(item)
		End Function
		...
		Friend Function Remove(item As BaseEvent) As Boolean Implements ICollection(Of BaseEvent).Remove
			If Contains(item) Then
				Dim result = Me.EventsBeatOrder(item.Beat.BeatOnly).Remove(item)
...
Namespace LevelElements
	...
	Public Class Decoration
		...
		Sub New()
		End Sub
		Friend Sub New(room As Rooms, asset As RDSprite, Optional depth As Integer = 0, Optional visible As Boolean = True)
			...
			Me.File = asset
		End Sub
		Sub New(room As Rooms, parent As RDSprite, id As String, Optional depth As Integer = 0, Optional visible As Boolean = True)
			Me.New(room, parent, depth, visible)
			_id = id
		End Sub
		<Obsolete("This function is obsolete and may be removed in the next release. Call CreateChildren<T>(RDBeat beatOnly) instead.")> Public Function CreateChildren(Of T As {BaseDecorationAction, New})(beatOnly As Single) As T
...
.Parent = Me
	...
	Public Class RDLevel
		...
		Public Sub New(items As IEnumerable(Of BaseEvent))
			...
			Next
		End Sub
		Public Function CreateDecoration(room As Rooms, sprite As RDSprite, Optional depth As Integer = 0, Optional visible As Boolean = True) As Decoration
			Assets.Add(sprite)
...
.Character = character
		...
		End Function
		Public Overloads Sub Add(beat As Single, item As BaseBeat)
			item._beat = Calculator.BeatOf(beat)
			Add(item)
		End Sub
