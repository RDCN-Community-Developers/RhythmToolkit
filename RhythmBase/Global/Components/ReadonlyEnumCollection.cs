...
namespace RhythmBase.Global.Components
{
	...
	public class ReadOnlyEnumCollection<TEnum> : IEnumerable<TEnum> where TEnum : struct, Enum
	{
		...
		{
			_bits = new ulong[source._bits.Length];
		...
		/// </summary>
		/// <param name="size">Number of 64-bit blocks to allocate for the internal bitset.</param>
		/// <param name="values">A list of enum values to include in the collection.</param>
		internal ReadOnlyEnumCollection(int size, params TEnum[] values)
		{
			_bits = new ulong[size];
			foreach (TEnum value in values)
			{
