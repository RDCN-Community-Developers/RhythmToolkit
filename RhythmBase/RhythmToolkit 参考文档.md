...
*`static`* **`Sprite`** `FromPath(string path)`
读取精灵图文件。
...
**`void`** `WriteJson(string path, SpriteOutputSettings settings)
以给定配置写入精灵图文件。
...
*`static`* **`Image`** `FromPath(string path)`
读取精灵图文件。

---

#### `PlaceHolder`
继承了`ISprite`接口的占位符类型，仅用于占位，缓解文件读取压力。
...
//导入关卡文件
RDLevel level = RDLevel.LoadFile("C:\\Document\\main.rdlevel", setting);
//导出关卡文件
level.SaveFile("levelEdited.rdlevel");
```
```VB
'定义配置: 精灵占位符
...
'导入关卡文件
Dim level As RDLevel = RDLevel.LoadFile("C:\Document\main.rdlevel", setting)
'导出关卡文件
level.SaveFile("levelEdited.rdlevel")
```
