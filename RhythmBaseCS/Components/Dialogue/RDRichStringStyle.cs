...
{
	public struct RDRichStringStyle
	{
		/// <summary>
		...
		/// <summary>
		/// Generates an XML tag representing the differences between two <see cref="RDRichStringStyle"/> instances.
		/// </summary>
		/// <param name="before">The initial <see cref="RDRichStringStyle"/> instance.</param>
		/// <param name="after">The modified <see cref="RDRichStringStyle"/> instance.</param>
		...
		private static string GetOpenTag(string name, string? arg = null) => arg is null ? $"<{name}>" : $"<{name}={arg}>";
		private static string GetCloseTag(string name) => $"</{name}>";
		private static void TryAddTag(ref string tag, string name, string? before, string? after)
		{
			if (before != after)
				tag += after is null
				? GetCloseTag(name)
		...
		private static void TryAddTag(ref string tag, string name, bool before, bool after)
		{
			if (before != after)
				tag += after
				? GetOpenTag(name)
		...
		/// <inheritdoc/>
		public static bool operator ==(RDRichStringStyle left, RDRichStringStyle right) =>
