...
namespace RhythmBase.Components.Easing
{
	...
	public class EaseValue(float originalValue, IEnumerable<EaseNode> nodes)
	{
		...
		public static EaseValue Fit(float originalValue, (float time, float value)[] values, float precision = 0.1f)
		{
			...
			return Fit([(0, originalValue), .. values], precision);
		}
		...
		/// <param name="precision">The precision for fitting the data points.</param>
		/// <returns>An <see cref="EaseValue"/> instance that fits the data points.</returns>
		public static EaseValue Fit((float time, float value)[] values, float precision = 0.1f) => Fit(values, eases, precision);
		/// <summary>
		...
		/// <param name="precision">The precision for fitting the data points.</param>
		/// <remarks>Special thanks to mfgujhgh for the algorithm!</remarks>
		/// <returns>An <see cref="EaseValue"/> instance that fits the data points.</returns>
		public static EaseValue Fit((float time, float value)[] values, EaseType[] easeTypes, float precision = 0.1f)
		{
			values = [.. values.OrderBy(x => x.time)];
			int[] steps = new int[values.Length], p = new int[values.Length];
			...
			for (int i = 1; i < values.Length; i++)
			{
				...
				for (int j = 0; j < i; j++)
				{
					EaseType besttype = EaseType.Linear;
