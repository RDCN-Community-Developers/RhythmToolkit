...
namespace RhythmBase.Global.Components.RichText
{
	...
	public class RDDialogueExchange : List<RDDialogueBlock>
	{
		...
		public static RDDialogueExchange Deserialize(string text) => [.. text.Split(['\r','\n'], StringSplitOptions.RemoveEmptyEntries).Select(RDDialogueBlock.Deserialize)];
		/// <summary>
		...
		public static RDDialogueExchange Deserialize(string text, ILookup<string, string> expressions) => [.. text.Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries).Select(i => RDDialogueBlock.Deserialize(i, expressions))];
		///<inheritdoc/>
