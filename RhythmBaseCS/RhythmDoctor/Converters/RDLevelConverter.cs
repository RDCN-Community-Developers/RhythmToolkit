	...
	internal class RDLevelConverter : JsonConverter<RDLevel>
	{
		...
		public override void WriteJson(JsonWriter writer, RDLevel? value, JsonSerializer serializer)
		{
			...
			if (settings.EnableGroupEvent)
			{
				...
				for (int i = 0; i < EventTypeUtils.GroupTypes.Count; i++)
					sb.AppendLine($"@{EventTypeUtils.GroupTypes[i].FullName}");
		...
		public override RDLevel ReadJson(JsonReader reader, Type objectType, RDLevel? existingValue, bool hasExistingValue, JsonSerializer serializer)
		{
			...
			try
			{
					...
					if ((settings.InactiveEventsHandling is not InactiveEventsHandling.Ignore) || !(item["active"]?.Value<bool>() ?? true))
					{
						...
						else
						{
							...
							if (ev != null)
							{
								...
								{
									if (ev is TagAction tagAction && Group.TryMatch(tagAction))
									{
										GroupTagCollection.Add(tagAction);
										handled = true;
									}
				...
				if (settings.EnableGroupEvent)
				{
					foreach (var tag in GroupTagCollection)
					{
						if (Group.TryParse(tag, typel, out Group? group))
						{
