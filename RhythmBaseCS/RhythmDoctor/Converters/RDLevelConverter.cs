	...
	internal class RDLevelConverter : JsonConverter<RDLevel>
	{
		...
		public override void WriteJson(JsonWriter writer, RDLevel? value, JsonSerializer serializer)
		{
			...
			if (settings.EnableGroupEvent)
			{
				...
				{
					if (!data.Contains(group._data))
					{
						data.Add(group._instance);
						group.DataId = id.ToString("X8");
						groupExtractEvents.AddRange(group);
						id++;
					}
				}
				if (groupExtractEvents.Any(i => i is Group group && groups.Contains(group)))
					throw new RhythmBaseException("Detected a group within itself. This is not allowed.");
				StringBuilder sb = new();
				sb.AppendLine(RhythmBaseGroupDataHeader);
				sb.AppendLine("/* Generated by RhythmBase */");
				for (int i = 0; i < data.Count; i++)
					sb.AppendLine(JsonConvert.SerializeObject(data[i], Formatting.None, AllInOneSerializer));
				groupExtractEvents.Add(new Comment() { Y = -1, Text = sb.ToString() });
			}
			else if (groups.Count > 0)
				throw new RhythmBaseException("An unexpected error occurred while processing the group event. Ensure that the group event is properly configured and adheres to the expected structure.");
			...
			if (settings.EnableGroupEvent)
			{
				foreach (var item in groupExtractEvents)
				{
					if (item is SetCrotchetsPerBar)
			...
			foreach (IBaseEvent item3 in value)
				if (item3 is Group group)
					writer.WriteRawValue(JsonConvert.SerializeObject(group.GenerateTagAction(), Formatting.None, AllInOneSerializer));
				else
			...
			try
			{
					...
					if ((settings.InactiveEventsHandling is not InactiveEventsHandling.Ignore) || !(item["active"]?.Value<bool>() ?? true))
					{
						...
						else
						{
							...
							if (ev != null)
							{
								...
								else if (settings.EnableGroupEvent)
								{
									if (ev is TagAction tagAction && Group.TryParse(tagAction, out Group? group))
									{
										GroupCollection.Add(group);
										outLevel.Add(group);
										handled = true;
									}
									else if (ev is Comment comment && Group.TryParse(comment, out JObject[]? data))
									{
										datal = data;
										handled = true;
									}
								}
								if (!handled) outLevel.Add(ev);
							}
						}
					}
				}
				if (settings.EnableGroupEvent)
				{
					foreach (var group in GroupCollection)
					{
						...
						int index = Convert.ToInt32(group.DataId, 16);
						group._data = datal[index < datal.Length ? index : throw new IndexOutOfRangeException()] ?? [];
						group.Flush();
					}
				}
