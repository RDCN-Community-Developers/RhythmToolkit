...
Imports Sprite.RhythmSprite.Exceptions
#Disable Warning CA1416
Namespace RhythmSprite
	Namespace Converters
		Class SpriteConverter(Of T As Sprite)
			Inherits JsonConverter(Of T)
			Private tempImageList As List(Of Bitmap)
			Public Sub New(imageList As List(Of Bitmap))
		...
		Class ClipListConverter
						...
						For Each item In value
							...
							For Each p In Properties
								...
								If p.Name = "Frames" Then
									M = CType(p.GetValue(item), List(Of Bitmap)).Select(Of UInteger)(Function(i) tempImageList.IndexOf(i))
									WriteSpace = False
								Else
									M = p.GetValue(item)
								End If
			...
			Public Overrides Function ReadJson(reader As JsonReader, objectType As Type, existingValue As List(Of Sprite.Clip), hasExistingValue As Boolean, serializer As JsonSerializer) As List(Of Sprite.Clip)
				...
				For Each item As Object In Arr
					...
					Dim ClipImageList As New List(Of Bitmap)
					For Each I In L
	...
	Public Class Sprite
		<JsonIgnore>
		Public Property Images As New List(Of Bitmap)
		<JsonIgnore>
		Public Property Images_Freeze As New List(Of Bitmap)
		<JsonIgnore>
		Public Property Images_Glow As Bitmap
		<JsonIgnore>
		Public Property Images_Outline As New List(Of Bitmap)
		...
		Public Class Clip
			...
			Public Property Frames As New List(Of Bitmap)
		End Class
		...
		Public Function ShouldSerializeRowPreviewOffset() As Boolean
			Return RowPreviewFrame IsNot Nothing
		End Function
		Public Shared Function ReadJson(path As String) As Sprite
			...
			End If
			Dim TempImages As New List(Of Bitmap)
			Dim Setting As New JsonSerializerSettings
			With Setting.Converters
				.Add(New Converters.SpriteConverter(Of Sprite)(TempImages))
			End With
			Dim ReadedSize As JToken = CType(JsonConvert.DeserializeObject(IO.File.ReadAllText(path), Setting), JObject)("size")
			Using img As New Bitmap(WithoutExtension + ".png")
				Dim size As New Vector2(ReadedSize(0), ReadedSize(1))
				TempImages = SplitImage(img, size)
			End Using
			Dim Temp = JsonConvert.DeserializeObject(Of Sprite)(IO.File.ReadAllText(path), Setting)
			Temp.Images = TempImages
			Return Temp
		End Function
		Private Shared Function SplitImage(img As Bitmap, size As Vector2, Optional inputMode As ImageInputOption = ImageInputOption.HORIZONTAL) As List(Of Bitmap)
			Dim Transparent As New Bitmap(CInt(size.X), CInt(size.Y))
			Dim L As New List(Of Bitmap)
			...
			Select Case inputMode
				Case ImageInputOption.HORIZONTAL
					For i = 0 To countSize.X * countSize.Y - 1
						Dim Area = New Rectangle((i Mod countSize.X) * size.X, (i \ countSize.X) * size.Y, size.X, size.Y)
						Dim tmpimg = img.Clone(Area, Imaging.PixelFormat.Format32bppArgb)
				...
				Case ImageInputOption.VERTICAL
					For i = 0 To countSize.X * countSize.Y - 1
						Dim Area = New Rectangle((i \ countSize.Y) * size.X, (i Mod countSize.Y) * size.Y, size.X, size.Y)
						Dim tmpimg = img.Clone(Area, Imaging.PixelFormat.Format32bppArgb)
						If TotallyTransparentImage(tmpimg) Then
							L.Add(Transparent)
						Else
			...
			Return L
		End Function
		Private Shared Function TotallyTransparentImage(img As Bitmap) As Boolean
			For x = 0 To img.Width - 1
				For y = 0 To img.Height - 1
					If img.GetPixel(x, y).A > 0 Then
						Return False
					End If
				Next
			Next
			Return True
		End Function
		Public Shared Function FromImage(img As Bitmap, size As Vector2, Optional inputMode As ImageInputOption = ImageInputOption.HORIZONTAL) As Sprite
		...
		Public Sub WriteJson(path As String, settings As SpriteOutputSettings)
			...
			If settings.Sort Then
				...
				For Each name In stringArray.Reverse
					Dim expression = Clips.FirstOrDefault(Function(i) i.Name = name)
					If expression IsNot Nothing Then
						Clips.Remove(expression)
						Clips.Insert(0, expression)
					End If
				Next
				Dim TempList As New List(Of Bitmap)
			...
			If settings.WithImage Then
				...
				Select Case settings.OutputMode
					Case SpriteOutputSettings.OutputModes.HORIZONTAL
						Dim png As New Bitmap(CInt(Math.Min(Images.Count, MaxCountSize.X) * Size.X),
											  CInt(Math.Ceiling(Images.Count / MaxCountSize.X) * Size.Y))
						Dim g As Graphics = Graphics.FromImage(png)
						For i = 0 To Images.Count - 1
							Dim index = Images
							g.DrawImage(Images(i), (i Mod MaxCountSize.X) * Size.X, (i \ MaxCountSize.X) * Size.Y)
						Next
						png.Save(WithoutExtension + ".png")
					Case SpriteOutputSettings.OutputModes.VERTICAL
						Dim png As New Bitmap(CInt(Math.Ceiling(Images.Count / MaxCountSize.Y) * Size.X),
											  CInt(Math.Min(Images.Count, MaxCountSize.Y) * Size.Y))
						Dim g As Graphics = Graphics.FromImage(png)
						For i = 0 To Images.Count - 1
							Dim index = Images
							g.DrawImage(Images(i), (i \ MaxCountSize.Y) * Size.X, (i Mod MaxCountSize.Y) * Size.Y)
						Next
						png.Save(WithoutExtension + ".png")
					Case SpriteOutputSettings.OutputModes.PACKED
						...
						Dim png As New Bitmap(CInt(Math.Min(Images.Count, BestCountSize.X) * Size.X),
											  CInt(Math.Ceiling(Images.Count / BestCountSize.X) * Size.Y))
						Dim g As Graphics = Graphics.FromImage(png)
						For i = 0 To Images.Count - 1
							Dim index = Images
							g.DrawImage(Images(i), (i Mod BestCountSize.X) * Size.X, (i \ BestCountSize.X) * Size.Y)
						Next
						png.Save(WithoutExtension + ".png")
				End Select
			End If
			...
			'	Dim Glowed As Bitmap = OutGlow(image)
		...
		End Sub
		Public Shared Function OutLine(image As Bitmap) As Bitmap
			Dim img As Bitmap = image.Clone()
			For x = 0 To img.Width - 1
				For y = 0 To img.Height - 1
					If image.GetPixel(x, y).A = 0 AndAlso
						(image.GetPixel(Math.Max(0, x - 1), y).A OrElse
						image.GetPixel(Math.Min(x + 1, img.Width - 1), y).A OrElse
						image.GetPixel(x, Math.Max(0, y - 1)).A OrElse
						image.GetPixel(x, Math.Min(y + 1, img.Width - 1)).A) Then
			...
			Return img
		End Function
		Public Shared Function OutGlow(image As Bitmap) As Bitmap
			Dim Img As Bitmap = image.Clone
			Dim radius = 4
		...
		Private Shared Function Kernel(radius As UInteger) As Single(,)
			...
			Return core
		End Function
		Private Shared Function GetPixelAlpha(radius As UInteger, x As Integer, y As Integer, image As Bitmap, core As Single(,), sum As Single) As Single
			...
			For i = If(0 <= x - radius, x - radius, 0) To If(x + radius <= image.Width - 1, x + radius, image.Width - 1)
				For j = If(0 <= y - radius, y - radius, 0) To If(y + radius <= image.Height - 1, y + radius, image.Height - 1)
					If i = x And j = y Then
					Else
						result += image.GetPixel(i, j).A * core(i + radius - x, j + radius - y)
					End If
				Next
			Next
			Return result / sum
